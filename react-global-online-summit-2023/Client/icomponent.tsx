  ...
  return fromJSX(k => {
    return <>
      {first.toJSX(newO1 => { 
        ...
        if (o1 == undefined && o2 == undefined) { console.log("race over, 1", [o1,o2,newO1]); k(newO1) }; 
        console.log("new state 1", [newO1,o2])
        setState([newO1,o2]) })}
      {second.toJSX(newO2 => { 
        console.log("second triggering with", [o1,[o2,newO2]])
        if (o1 == undefined && o2 == undefined) { console.log("race over, 2", [o1,o2,newO2]); k(newO2); } 
...
export function all<o1,o2>(first:IComponent<o1>, second:IComponent<o2>) : IComponent<[o1,o2]> {
  ...
  return fromJSX(k => {
    return <>
      {first.toJSX(newO1 => { if (o1 == undefined && o2 != undefined) k([newO1,o2]);setState([newO1,o2]) })}
      {second.toJSX(newO2 => { if (o1 != undefined && o2 == undefined) k([o1,newO2]);setState([o1,newO2]) })}
    </>
  })
}

function promiseToJSX<o>(p:Promise<o>,k:Fun<o,void>) : JSX.Element {   
  const [state,setState] = useState<"not invoked"|"invoked"|"completed"|"failed">("not invoked")
  useEffect(() => {
    if (state == "not invoked") {
      setState("invoked")
      p.then(o => {  k(o); setState("completed") }).catch(_ => setState("failed"))
    }
    return () => { if (state != "completed") setState("failed") }
  }, [p,k]);

  return <></>
}

export const fromPromise = <o,>(p:Promise<o>): IComponent<o> =>
  ({
    toJSX: k => promiseToJSX(p,k),
    map: <o2,>(f: Fun<o, o2>): IComponent<o2> => fromJSX(k2 => promiseToJSX(p,o => k2(f(o)))),
    race: function<o2,>(this:IComponent<o>, other:IComponent<o2>): IComponent<o | o2> { return race(this,other)},
    all: function<o2,>(this:IComponent<o>, other:IComponent<o2>): IComponent<[o,o2]> { return all(this,other)}
  })
